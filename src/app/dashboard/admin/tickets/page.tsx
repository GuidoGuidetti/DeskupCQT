...
import { PlusCircleIcon, PaperclipIcon, ClipboardListIcon, EditIcon } from 'lucide-react';
...
export default async function AdminAllTicketsPage({
  ...
  const customerFilter = params.customer ? parseInt(params.customer) : undefined;

  const tickets = await prisma.ticket.findMany({
    where: {
      ...(showOnlyOpen ? { tick_status: { not: 'closed' } } : {}),
      ...(customerFilter ? { tick_cust_id: customerFilter } : {}),
    },
    include: {
      user: true,
      customer: true,
  ...
  // Get customers for filter (only if admin or partner)
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Tutti i Tickets</h2>
            <p className="text-gray-600 mt-1">Visualizza e gestisci tutti i tickets del sistema</p>
          </div>
          <Link
        ...
        <div className="mb-4 flex gap-4 items-center flex-wrap">
          <div className="flex gap-2">
            <Link
              href={`/dashboard/admin/tickets${customerFilter ? `?customer=${customerFilter}` : ''}`}
            ...
            <Link
              href={`/dashboard/admin/tickets?status=open${customerFilter ? `&customer=${customerFilter}` : ''}`}
              className={`px-4 py-2 rounded-lg transition-colors ${
                showOnlyOpen
                  ? 'bg-primary-600 text-white'
                  : 'bg-white text-gray-700 border border-gray-300 hover:bg-gray-50'
          ...
          <CustomerFilter customers={customers} currentCustomer={customerFilter} />
        </div>

        {tickets.length === 0 ? (
          <div className="bg-white shadow-sm rounded-lg border border-gray-200 p-12 text-center">
            <p className="text-gray-600 mb-4">Nessun ticket presente nel sistema.</p>
            <Link
              href="/dashboard/user/tickets/new"
                  ...
                  <tr key={ticket.tick_id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-4 text-sm">
                      <div className="flex items-center gap-1">
                        <Link
                          href={`/dashboard/admin/tickets/${ticket.tick_id}/activities`}
                        ...
                        <Link
                          href={`/dashboard/admin/tickets/${ticket.tick_id}/edit`}
